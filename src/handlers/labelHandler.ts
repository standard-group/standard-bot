...
export async function handleLabelAction(
  ...
  const {
    action,
    delay = botConfig.default?.[typeof config === 'string' ? config : config.action]?.delay ||
      '0s',
    comment,
    message
  } =
    ...
    if (action === 'close') {
      if (comment !== false) {
        const body = replaceVars(
          comment || botConfig.default?.close?.comment,
          {
            DELAY: delay,
            LABEL: label
          }
      ...
      // Post a comment with the specified message.
      const body = message || '';
      if (body) {
