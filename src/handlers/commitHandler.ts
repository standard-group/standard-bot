...
export async function handlePullRequestSynchronize(
  ...
  console.log(
    `[+] Pull Request #${issue_number} synchronized. Checking commit actions.`
  );
  ...
  if (botConfig.commits && Array.isArray(botConfig.commits)) {
    for (const commitConfig of botConfig.commits) {
      ...
      try {
        if (
          typeof commitConfig.pattern === "string" &&
          commitConfig.pattern.startsWith("/") &&
          commitConfig.pattern.endsWith("/i")
        ) {
          ...
          patternRegex = new RegExp(
            commitConfig.pattern.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
            "i"
          );
        } else {
          console.warn(
            `Invalid pattern type for commit action: ${commitConfig.pattern}`
          );
          continue;
        }
      } catch (e) {
        console.error(
          `Invalid regex pattern in config for commit action: ${commitConfig.pattern}`,
          e
        );
        continue;
      }
      ...
      for (const commit of commits) {
        if (patternRegex.test(commit.commit.message)) {
          if (
            commitConfig.user &&
            commitConfig.user !== commit.author?.login &&
            commitConfig.user !== commit.committer?.login
          ) {
          ...
          try {
            if (
              commitConfig.action === "label" &&
              Array.isArray(commitConfig.labels)
            ) {
              ...
              console.log(
                `[Action] Applied labels ${commitConfig.labels.join(
                  ", "
                )} to PR #${issue_number} from commit "${commit.sha}".`
              );
            } else {
              console.warn(
                `Unsupported commit action "${commitConfig.action}" or invalid labels array.`
              );
            }
          } catch (error: any) {
            console.error(
              `Error handling commit action for PR #${issue_number} from commit "${commit.sha}":`,
              error.message
            );
          }
        }
