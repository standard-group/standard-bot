...
export async function handleCommentAction(
  ...
  if (!issue_number) {
    console.warn(
      `Could not find issue/pull request number for comment action.`
    );
    return;
  }
  ...
  if (botConfig.comments && Array.isArray(botConfig.comments)) {
    for (const commentConfig of botConfig.comments) {
      ...
      try {
        if (
          typeof commentConfig.pattern === "string" &&
          commentConfig.pattern.startsWith("/") &&
          commentConfig.pattern.endsWith("/i")
        ) {
          ...
          patternRegex = new RegExp(
            commentConfig.pattern.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
            "i"
          );
        } else {
          console.warn(
            `Invalid pattern type for comment action: ${commentConfig.pattern}`
          );
          continue;
        }
      } catch (e) {
        console.error(
          `Invalid regex pattern in config for comment action: ${commentConfig.pattern}`,
          e
        );
        continue;
      }
      ...
      if (patternRegex.test(commentBody)) {
        try {
          if (
            commentConfig.action === "label" &&
            Array.isArray(commentConfig.labels)
          ) {
            ...
            console.log(
              `[Action] Applied labels ${commentConfig.labels.join(
                ", "
              )} to #${issue_number} from comment.`
            );
            ...
            console.log(
              `[Action] Deleted comment ID ${commentId} on #${issue_number}.`
            );
          } else {
            console.warn(
              `Unsupported comment action "${commentConfig.action}" or invalid labels array.`
            );
          }
        } catch (error: any) {
          console.error(
            `Error handling comment action for #${issue_number}:`,
            error.message
          );
        }
      }
